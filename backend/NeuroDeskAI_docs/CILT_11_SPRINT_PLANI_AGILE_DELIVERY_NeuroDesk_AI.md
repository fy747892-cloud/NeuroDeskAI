...
Sprint 6 hazirlik notu: Sprint 5 sonunda onaylanan `ai_action_approvals` kayitlari
`status=approved`, `action_type=task` ve `approved_payload` alanlariyla Task modulu tarafindan
consume edilmelidir. Task service, onay kaydini tenant/organization kapsaminda yeniden okumali,
payload'i tekrar validate etmeli, task kaydini olusturduktan sonra audit log yazmali ve ayni
approval'in ikinci kez task uretmesini engelleyecek idempotency kontrolu eklemelidir.
