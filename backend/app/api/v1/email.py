...
@router.post("/messages/{message_id}/mark-replied", response_model=EmailMessageOut)
async def mark_email_message_replied(
    ...
    if current_user.organization_id is None:
        raise NotFoundError("Current organization not found.")
    repository = EmailMessageRepository(db)
    ...
    message = await repository.mark_replied(message=message)
    await db.commit()
    return message
